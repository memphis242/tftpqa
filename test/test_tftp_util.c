...
void test_util_is_valid_filename_char_accepts_alphanumeric(void)
{
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( 'a' ) );
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( 'Z' ) );
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( '0' ) );
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( '.' ) );
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( '-' ) );
   TEST_ASSERT_TRUE( tftpqa_util_is_valid_filename_char( '_' ) );
}

void test_util_is_valid_filename_char_rejects_separators(void)
{
   TEST_ASSERT_FALSE( tftpqa_util_is_valid_filename_char( '/' ) );
   TEST_ASSERT_FALSE( tftpqa_util_is_valid_filename_char( '\\' ) );
}

void test_util_is_valid_filename_char_rejects_control_chars(void)
{
   TEST_ASSERT_FALSE( tftpqa_util_is_valid_filename_char( '\0' ) );
   TEST_ASSERT_FALSE( tftpqa_util_is_valid_filename_char( '\n' ) );
   TEST_ASSERT_FALSE( tftpqa_util_is_valid_filename_char( '\x7F' ) );
}

void test_util_create_ephemeral_socket_succeeds(void)
{
   struct sockaddr_in addr;
   int sfd = tftpqa_util_create_ephemeral_udp_socket( &addr );
...
void test_util_set_recv_timeout_succeeds(void)
{
   ...
   int rc = tftpqa_util_set_recv_timeout( sfd, 2 );
...
void test_util_netascii_bare_lf_becomes_crlf(void)
{
   ...
   size_t n = tftpqa_util_octet_to_netascii(in, sizeof(in) - 1, out, sizeof out, &pending_cr);
...
void test_util_netascii_bare_cr_becomes_cr_nul(void)
{
   ...
   uint8_t out[16];
   bool pending_cr = false;
   size_t n = tftpqa_util_octet_to_netascii(in, sizeof in, out, sizeof out, &pending_cr);
...
void test_util_netascii_pending_cr_across_boundary(void)
{
   ...
   uint8_t out[16];
   bool pending_cr = false;
   size_t n1 = tftpqa_util_octet_to_netascii(in1, sizeof in1, out, sizeof out, &pending_cr);
   TEST_ASSERT_EQUAL_size_t( 2, n1 ); // 'a' + '\r'
   TEST_ASSERT_TRUE( pending_cr );

   // Next chunk starts with \n -- should complete the \r\n
   uint8_t in2[] = { '\n', 'b' };
   size_t n2 = tftpqa_util_octet_to_netascii(in2, sizeof in2, out, sizeof out, &pending_cr);
...
void test_util_netascii_no_special_chars(void)
{
   ...
   uint8_t out[16];
   bool pending_cr = false;
   size_t n = tftpqa_util_octet_to_netascii(in, 5, out, sizeof out, &pending_cr);
...
void test_util_netascii_empty_input(void)
{
   uint8_t out[8];
   bool pending_cr = false;
   size_t n = tftpqa_util_octet_to_netascii(NULL, 0, out, sizeof out, &pending_cr);
...
void test_util_netascii_to_octet_crlf_becomes_lf(void)
{
   ...
   uint8_t out[8];
   bool pending_cr = false;
   size_t n = tftpqa_util_netascii_to_octet(in, sizeof in, out, sizeof out, &pending_cr);
...
void test_util_netascii_to_octet_pending_cr_boundary(void)
{
   ...
   uint8_t out[8];
   bool pending_cr = false;
   size_t n1 = tftpqa_util_netascii_to_octet(in1, sizeof in1, out, sizeof out, &pending_cr);
   TEST_ASSERT_EQUAL_size_t( 1, n1 ); // just 'x', \r is pending
   TEST_ASSERT_TRUE( pending_cr );

   // Second chunk: \n completes the \r\n -> \n
   uint8_t in2[] = { '\n', 'y' };
   size_t n2 = tftpqa_util_netascii_to_octet(in2, sizeof in2, out, sizeof out, &pending_cr);
   TEST_ASSERT_EQUAL_size_t( 2, n2 ); // '\n' + 'y'
   TEST_ASSERT_EQUAL_UINT8( '\n', out[0] );
   TEST_ASSERT_EQUAL_UINT8( 'y', out[1] );
   TEST_ASSERT_FALSE( pending_cr );
...
void test_util_netascii_to_octet_no_special(void)
{
   ...
   uint8_t out[8];
   bool pending_cr = false;
   size_t n = tftpqa_util_netascii_to_octet(in, 5, out, sizeof out, &pending_cr);
   TEST_ASSERT_EQUAL_size_t( 5, n );
   TEST_ASSERT_EQUAL_MEMORY( "hello", out, 5 );
   TEST_ASSERT_FALSE( pending_cr );
}
...
void test_chroot_and_drop_non_root_succeeds(void)
{
   ...
   int rc = tftpqa_util_chroot_and_drop("/tmp", "nobody");
   TEST_ASSERT_EQUAL_INT( 0, rc );
...
void test_chroot_and_drop_bad_dir_fails(void)
{
   int rc = tftpqa_util_chroot_and_drop("/nonexistent_dir_12345", "nobody");
...
void test_util_create_udp_socket_in_range_succeeds(void)
{
   ...
   int sfd = tftpqa_util_create_udp_socket_in_range(49200, 49210, &bound_addr);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, sfd );
...
{
   struct sockaddr_in bound_addr = {0};
   int sfd = tftpqa_util_create_udp_socket_in_range(49220, 49220, &bound_addr);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, sfd );
...
void test_util_create_udp_socket_in_range_all_busy(void)
{
   ...
   int sfd = tftpqa_util_create_udp_socket_in_range(base, (uint16_t)(base + 2), NULL);
...
void test_util_suspicious_text_clean_ascii(void)
{
   ...
   TEST_ASSERT_EQUAL_INT( TFTP_TEXT_CLEAN, tftpqa_util_check_text_bytes(data, sizeof(data) - 1) );
}
...
void test_util_suspicious_text_allowed_controls(void)
{
   ...
   TEST_ASSERT_EQUAL_INT( TFTP_TEXT_CLEAN, tftpqa_util_check_text_bytes(data, sizeof(data)) );
}
...
void test_util_suspicious_text_standalone_nul(void)
{
   ...
   TEST_ASSERT_EQUAL_INT( TFTP_TEXT_SUSPICIOUS, tftpqa_util_check_text_bytes(data, sizeof(data)) );
}
...
void test_util_suspicious_text_empty_buffer(void)
{
   TEST_ASSERT_EQUAL_INT( TFTP_TEXT_CLEAN, tftpqa_util_check_text_bytes(NULL, 0) );
}
...
void test_util_text_check_valid_utf8_2byte(void)
{
   ...
   TEST_ASSERT_EQUAL_INT( TFTP_TEXT_HAS_UTF8, tftpqa_util_check_text_bytes(data, sizeof(data)) );
}
...
void test_util_check_read_perms_world_readable(void)
{
   char path[] = "/tmp/tftpqa_perm_rd_XXXXXX";
   int fd = perm_test_make_tmpfile(path, 0644);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, fd );

   mode_t observed = 0;
   enum TFTPTestUtil_PermCheck rc = tftpqa_util_check_read_perms(fd, &observed);
...
void test_util_check_read_perms_not_world_readable(void)
{
   char path[] = "/tmp/tftpqa_perm_nr_XXXXXX";
...
void test_util_check_read_perms_setuid_rejected(void)
{
   char path[] = "/tmp/tftpqa_perm_su_XXXXXX";
...
void test_util_check_write_perms_world_writable(void)
{
   char path[] = "/tmp/tftpqa_perm_ww_XXXXXX";
   int fd = perm_test_make_tmpfile(path, 0666);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, fd );

   mode_t observed = 0;
   enum TFTPTestUtil_PermCheck rc = tftpqa_util_check_write_perms(fd, &observed);
   TEST_ASSERT_EQUAL_INT( TFTP_PERM_OK, rc );
...
void test_util_check_write_perms_not_world_writable(void)
{
   char path[] = "/tmp/tftpqa_perm_nw_XXXXXX";
...
void test_util_open_for_read_rejects_symlink(void)
{
   ...
   errno = 0;
   int fd = tftpqa_util_open_for_read(linkpath);
...
void test_util_open_for_write_creates_with_mode(void)
{
   char path[] = "/tmp/tftpqa_open_cr_XXXXXX";
   ...
   bool created = false;
   int fd = tftpqa_util_open_for_write(path, 0666, &created);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, fd );
...
void test_util_open_for_write_overwrites_existing(void)
{
   char path[] = "/tmp/tftpqa_open_ov_XXXXXX";
   int fd = perm_test_make_tmpfile(path, 0666);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, fd );
   ...
   int wfd = tftpqa_util_open_for_write(path, 0666, &created);
...
void test_util_open_for_write_create_mode_stripped_by_umask(void)
{
   char path[] = "/tmp/tftpqa_open_um_XXXXXX";
   int dummy = mkstemp(path);
   TEST_ASSERT_GREATER_OR_EQUAL_INT( 0, dummy );
   (void)close(dummy);
   TEST_ASSERT_EQUAL_INT( 0, unlink(path) );
