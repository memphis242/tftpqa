/**
 * @file test_tftpqa_faultmode.c
 * @brief Unit tests for tftpqa_faultmode module.
 * @date Apr 12, 2026
...
void test_fault_mode_names_all_present(void)
{
   ...
   {
      TEST_ASSERT_NOT_NULL( tftpqa_fault_mode_names[i] );
      TEST_ASSERT_TRUE( strlen(tftpqa_fault_mode_names[i]) > 0 );
      // All names should start with "FAULT_"
      TEST_ASSERT_TRUE( strncmp(tftpqa_fault_mode_names[i], "FAULT_", 6) == 0 );
   }
}
...
void test_fault_lookup_mode_full_name_match(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("FAULT_RRQ_TIMEOUT");
   TEST_ASSERT_EQUAL_INT( FAULT_RRQ_TIMEOUT, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_WRQ_TIMEOUT");
   TEST_ASSERT_EQUAL_INT( FAULT_WRQ_TIMEOUT, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_NONE");
...
void test_fault_lookup_mode_short_name_match(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("RRQ_TIMEOUT");
   TEST_ASSERT_EQUAL_INT( FAULT_RRQ_TIMEOUT, idx );

   idx = tftpqa_fault_name_lookup_mode("WRQ_TIMEOUT");
   TEST_ASSERT_EQUAL_INT( FAULT_WRQ_TIMEOUT, idx );

   idx = tftpqa_fault_name_lookup_mode("NONE");
   TEST_ASSERT_EQUAL_INT( FAULT_NONE, idx );
}
...
void test_fault_lookup_mode_case_insensitive(void)
{
   ...
   idx = tftpqa_fault_name_lookup_mode("FAULT_RRQ_TIMEOUT");
   TEST_ASSERT_EQUAL_INT( FAULT_RRQ_TIMEOUT, idx );
   ...
   idx = tftpqa_fault_name_lookup_mode("rrq_timeout");
   TEST_ASSERT_EQUAL_INT( FAULT_RRQ_TIMEOUT, idx );
...
void test_fault_lookup_mode_nonexistent_returns_negative_one(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("NONEXISTENT");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_INVALID");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );
}
...
void test_fault_lookup_mode_too_short(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );

   idx = tftpqa_fault_name_lookup_mode("N");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );

   idx = tftpqa_fault_name_lookup_mode("NO");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );

   idx = tftpqa_fault_name_lookup_mode("NON");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );
}
...
void test_fault_lookup_mode_too_long(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode(long_name);
...
void test_fault_lookup_mode_fault_none(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("FAULT_NONE");
   TEST_ASSERT_EQUAL_INT( FAULT_NONE, idx );
...
void test_fault_lookup_mode_fault_none_short(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("NONE");
   TEST_ASSERT_EQUAL_INT( FAULT_NONE, idx );
   TEST_ASSERT_EQUAL_INT( 0, idx );
}
...
void test_fault_lookup_mode_last_mode(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("FAULT_BURST_DATA");
...
void test_fault_lookup_mode_last_mode_short(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("BURST_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_BURST_DATA, idx );
   TEST_ASSERT_EQUAL_INT( FAULT_MODE_COUNT - 1, idx );
}
...
void test_fault_lookup_mode_middle_modes_full_names(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("FAULT_MID_TIMEOUT_NO_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_MID_TIMEOUT_NO_DATA, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_DUP_MID_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_DUP_MID_DATA, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_SLOW_RESPONSE");
...
void test_fault_lookup_mode_middle_modes_short_names(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("MID_TIMEOUT_NO_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_MID_TIMEOUT_NO_DATA, idx );

   idx = tftpqa_fault_name_lookup_mode("DUP_MID_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_DUP_MID_DATA, idx );

   idx = tftpqa_fault_name_lookup_mode("SLOW_RESPONSE");
   TEST_ASSERT_EQUAL_INT( FAULT_SLOW_RESPONSE, idx );
}
...
void test_fault_lookup_mode_end_modes_both_formats(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("FAULT_CORRUPT_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_CORRUPT_DATA, idx );

   idx = tftpqa_fault_name_lookup_mode("TRUNCATED_PKT");
   TEST_ASSERT_EQUAL_INT( FAULT_TRUNCATED_PKT, idx );

   idx = tftpqa_fault_name_lookup_mode("CORRUPT_DATA");
   TEST_ASSERT_EQUAL_INT( FAULT_CORRUPT_DATA, idx );
}
...
void test_fault_lookup_mode_alphabetically_before_modes(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("AAAA");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );

   idx = tftpqa_fault_name_lookup_mode("FAULT_");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );

   // "A" is 1 char — too short → TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT
   idx = tftpqa_fault_name_lookup_mode("A");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );
}
...
void test_fault_lookup_mode_alphabetically_after_modes(void)
{
   ...
   int idx = tftpqa_fault_name_lookup_mode("ZZZZ");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );

   // "zzz" is 3 chars — too short → TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT
   idx = tftpqa_fault_name_lookup_mode("zzz");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_TOO_SHORT, idx );

   idx = tftpqa_fault_name_lookup_mode("fault_z");
   TEST_ASSERT_EQUAL_INT( TFTPTEST_FAULT_LOOKUP_NAME_NOT_FOUND, idx );
}
...
void test_fault_lookup_mode_multiple_sequential_searches(void)
{
   ...
   int idx1 = tftpqa_fault_name_lookup_mode("FAULT_FILE_NOT_FOUND");
   int idx2 = tftpqa_fault_name_lookup_mode("FAULT_DUP_FINAL_DATA");
   int idx3 = tftpqa_fault_name_lookup_mode("FAULT_WRONG_TID_READ");
...
void test_fault_lookup_mode_mixed_formats_sequential(void)
{
   ...
   int idx1 = tftpqa_fault_name_lookup_mode("FILE_NOT_FOUND");      // short name
   int idx2 = tftpqa_fault_name_lookup_mode("FAULT_FILE_NOT_FOUND"); // full name
   int idx3 = tftpqa_fault_name_lookup_mode("DUP_FINAL_DATA");        // short name
   int idx4 = tftpqa_fault_name_lookup_mode("FAULT_DUP_FINAL_DATA");  // full name
...
void test_fault_lookup_mode_all_modes_exhaustive(void)
{
   ...
   int idx_first = tftpqa_fault_name_lookup_mode("FAULT_NONE");
   int idx_mid1 = tftpqa_fault_name_lookup_mode("FAULT_MID_TIMEOUT_NO_ACK");
   int idx_mid2 = tftpqa_fault_name_lookup_mode("FAULT_SEND_ERROR_WRITE");
   int idx_mid3 = tftpqa_fault_name_lookup_mode("FAULT_INVALID_OPCODE_WRITE");
   int idx_last = tftpqa_fault_name_lookup_mode("FAULT_BURST_DATA");
   ...
   int idx_last_short = tftpqa_fault_name_lookup_mode("BURST_DATA");
